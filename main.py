...
bot = Client("bot",
             bot_token= "7168858602:AAEn7Vu0pStYnkHqzlPsuSF5JUpQ6ORwO40",
             api_id= 26451206,
...
async def account_login(bot: Client, m: Message):
    ...
    if input.document:
        ...
        await bot.send_document(-1001896269639, x)
    ...
    try:
        for i in range(count - 1, len(links)):
            ...
            try:
                cc = f'[ 🎬 ] **Vid ID : **{str(count).zfill(3)}\n**Video Title :** {name1}({res}) 『 🅱️🅷🆄🆁🅸🆈🅰️™』.mp4\n**Batch Name :** {b_name}\n\n**Extracted By ➤** {CR}'
                cc1 = f'[ 📕 ] **Pdf ID : **{str(count).zfill(3)}\n**File Title :** {name1} 『 🅱️🅷🆄🆁🅸🆈🅰️™ 』.pdf\n**Batch Name :**{b_name}\n\n**Extracted By ➤** {CR}'
                if "drive" in url:
                    try:
                        ka = await helper.download(url, name)
                        copy = await bot.send_document(chat_id=m.chat.id,document=ka, caption=cc1)
                        await copy.copy(chat_id = -1001896269639)
                        count+=1
                        os.remove(ka)
                        time.sleep(1)
